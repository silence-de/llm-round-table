...
- [ ] 第一阶段不改动原始 transcript 存储策略，只新增 reflection artifact，避免把 memory 项目扩大成存储重构
...
- [ ] 在 follow-up session 初始化时优先注入 reflection-first context；先做增量接管，不要求一次性替换所有历史继承逻辑
...
- [ ] 先落库与内部查询接口，是否暴露独立 lessons API 作为第二阶段决定
...
- [ ] 第一阶段仅对二元预测实现 Brier 分数；连续值预测单独记录，后续再决定 CRPS 等指标
