...
**当前代码缺口（GPT 视角）**
- 缺少 claim-level gate：extraction 仍可能被 summary 当作 verification 使用（`browser-verify.ts` 已有 `extractionMethod`/`extractionQuality`，但下游 summary 未做门控）。
- 缺少 `allowed / disallowed / gap-only` 声明策略的系统化实现。
- 缺少内容级 injection suspect 标注与降权逻辑。

**P0 开发优先级（GPT 视角）**
- 在 summary 之前增加 claim gate，区分 `allowed_without_evidence` / `requires_evidence` / `gap_only`。
- 为每条 claim 增加：`claimType`、`evidenceRefs`、`verificationStatus`、`uncertaintyLabel`。
- 统一替换误导性词汇，确保 UI/PDF 中不把提取结果说成 verified。

**P1 开发优先级（GPT 视角）**
- 增加 untrusted content scan：提示注入可疑标记、suspicious instructions 摘要、降权或阻断进入最终 summary。
- 为缺失 research 建立标准降级决策树。

**P2 开发优先级（GPT 视��）**
- 引入最小 trust classification：source-level / claim-level / summary-level。
- 高风险主题启用更严格人工复核。

**验收指标（GPT 视角）**
- 测试：extraction 不等于 verification 的断言测试、research 缺失场景降级测试、注入可疑页面标记测试。
- UI 验收：不出现误导性的 verified copy，claim 可追溯到 evidence refs。
- 运营指标：`gap_only_claim_ratio`、`research_degraded_rate`、`suspicious_content_flagged_rate`。
