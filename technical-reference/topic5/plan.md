...
- [ ] buffer 实现：Redis List 或内存 Map（key: `sse_buffer:{sessionId}`，TTL: 30 分钟）
- [ ] SSE 响应头增加 `Last-Event-ID` 支持
...
### 任务
- [ ] 定义幂等键规则：
  - summary 写入：`idempotency_key = sha256(session_id + phase + round_number)`
  - ledger checkpoint：`idempotency_key = sha256(session_id + phase + ledger_version)`
  - PDF export：`idempotency_key = sha256(session_id + export_timestamp_minute)`
- [ ] 在 `schema.ts` 中为 summaries / checkpoints / exports 表增加 `idempotency_key UNIQUE` 约束
- [ ] 在 `repository.ts` 中所有写入函数改为 `INSERT ... ON CONFLICT (idempotency_key) DO NOTHING`
...
- [ ] 客户端 SSE 连接逻辑：
  ...
  - 重连时在 URL 中携带 `?lastEventId={id}` 或通过 `EventSource` 的 `Last-Event-ID` header
- [ ] 服务端重连处理：
  - 读取 `lastEventId` 参数
  - 调用 `getEventsSince(sessionId, lastEventId)` 补发缺失事件
  - 补发完成后切换为实时流模式
- [ ] 补发事件标注 `type: 'replay'`，客户端可区分实时事件与补发事件
...
- [ ] 实现 `replaySession(sessionId, fromCheckpointId)` 函数：
  ...
  - 按 eventId 顺序重放 buffer 中的事件（不重跑 LLM）
  - 输出重放日志到文件
